...
function AddUser({ add, setAdd, setDidChangeData }) {
  ...
  const onRegister = async () => {
    ...
    try {
      const res = await fetch("/api/admin/add", options);
  ...
  return (
    <section className={add ? "edit-modal" : "edit-modal-close"}>
      <div className="edit-modal-wrapper">
        <div className="close-container">
          <img src={Close} alt="Close" onClick={() => setAdd(!add)} />
        </div>
        <Box
          sx={{
            display: "flex",
